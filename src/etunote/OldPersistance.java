...
public class OldPersistance {
	...
	public void SaveNote(Note n, String f) {
		// Cr�ation d'un nouvel Element note que l'on ajoute en tant qu'Element
		// de racine
		...
		Attribute created_at = new Attribute("created_at", String.valueOf(n
				.getCreated_at()));
		note.setAttribute(created_at);

		Attribute modified_at = new Attribute("modified_at", String.valueOf(n
				.getModified_at()));
		...
		while (iter.hasNext()) {
			...
			if (c instanceof Title) {
				...
				Attribute level = new Attribute("level",
						String.valueOf(((Title) c).getLevel()));
				contenu.setAttribute(level);
				Attribute position = new Attribute("position",
						String.valueOf(((Title) c).getPosition()));
				...
				Attribute position = new Attribute("position",
						String.valueOf(((Paragraph) c).getPosition()));
				contenu.setAttribute(position);
	...
	public void LoadNote(String f) {
		...
		// On cr�e une Liste contenant tous les noeuds "note" de l'Element
		// racine
